...
<Note>
  ...
  You can leave it blank only if the Gateway can reach the CA host over SMB (TCP 445), which lets Infisical discover the name for you. Infisical checks the name when you save, so a wrong or unreachable name fails right away instead of creating a broken CA.
</Note>
  ...
  <Tab title="External CA">
    <Steps>
      ...
      <Step title="Create a New Microsoft ADCS Certificate Authority">
        ...
        Infisical contacts the CA through the Gateway when you save, so a wrong name or an unreachable host is rejected here rather than surfacing later as a CA that cannot list templates or issue.
      </Step>
